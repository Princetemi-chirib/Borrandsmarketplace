    ...
    <div className="min-h-screen bg-gray-50">
      <main className="mx-auto w-full max-w-5xl px-4 py-6">
        <div className="mb-4 flex items-center justify-between">
          <button onClick={() => router.back()} className="inline-flex items-center gap-1 text-sm font-semibold text-blue-700">
        ...
        <div className="mb-5 overflow-hidden rounded-2xl border border-gray-200 bg-white">
          ...
          <div className="p-4">
            ...
            <p className="mt-1 text-sm text-gray-600">{restaurant?.description || 'Fresh meals for campus life.'}</p>
          </div>
        </div>
            ...
            <div key={item.id} className="rounded-2xl border border-gray-200 bg-white p-4">
              <div className="mb-2 flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="text-sm text-gray-600">N{Number(item.price).toLocaleString()}</p>
                </div>
                <div className="flex items-center gap-1">
                  <button onClick={() => sub(item)} className="rounded-md border border-gray-200 p-1"><Minus className="h-4 w-4" /></button>
                  <span className="w-6 text-center text-sm">{qty(item.id)}</span>

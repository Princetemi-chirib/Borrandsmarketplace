      ...
      <main className="mx-auto w-full max-w-5xl px-4 py-6">
        <div className="mb-5 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">Browse and add meals before login.</p>
          </div>
          <Link href="/entry" className="text-sm font-semibold text-blue-700 hover:text-blue-800">
            Back
          </Link>
        </div>
        ...
        <div className="mb-5 rounded-xl border border-gray-200 bg-white p-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-200 py-2 pl-9 pr-3 text-sm focus:border-red-400 focus:outline-none"
            />
          </div>
        </div>

        {loading ? (
          <div className="py-12 text-center text-sm text-gray-500">Loading restaurants...</div>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((r) => {
              ...
              return (
                <Link key={r.id} href={`/restaurants/${r.id}`} className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                  {image ? (
                  ...
                  <div className="p-4">
                    ...
                    <p className="mt-1 text-xs text-gray-600">{cuisine}</p>
